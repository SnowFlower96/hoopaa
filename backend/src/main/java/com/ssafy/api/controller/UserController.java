...
public class UserController {
    ...
    public ResponseEntity<? extends BaseResponseBody> login(@RequestBody @ApiParam(value = "로그인 정보", required = true) UserLoginPostReq loginInfo) {
        ...
        if (!user.isEm_auth()) ResponseEntity.status(401).body(BaseResponseBody.of(404, "Unauthorized User"));

        // 탈퇴된 회원이면 로그인 실패
        if (user.getLeave_dt() != null) ResponseEntity.status(401).body(BaseResponseBody.of(404, "Resigned User"));
    ...
    @ApiOperation(value = "이메일 인증 완료", notes = "로그인 필요 서비스 사용을 위한 이메일 인증")
    @ApiResponses({
            @ApiResponse(code = 200, message = "성공"),
            @ApiResponse(code = 401, message = "인증 실패"),
            @ApiResponse(code = 404, message = "사용자 없음"),
