<template>
  <div v-if="imgTF" class="startImg">
    ...
    <router-link to="/detailSessionView"
      ><button>세부세션 가기</button></router-link
    >
  ...
  >
    ...
    <div class="call-to-moderator">
      ...
      <div
        class="call-to-moderator-inner call-to-moderator-center"
        :style="customCaroselStyle"
      >
...
export default {
  ...
  async created() {
    ...
    this.room.session.on("streamCreated", ({ stream }) => {
      ...
      console.log(clientData.split("/")[0], this.room.session.sessionId);
      console.log(clientData.split("/")[0] == this.room.session.sessionId);
      if (clientData.split("/")[0] == this.room.session.sessionId) {
        this.room.host = subscriber;
        console.log("hihiihhih", this.room.host);
  ...
  methods: {
    ...
    // 찬성 참여
    positionAgree() {
      (this.position = "agree"), console.log(this.$refs);
