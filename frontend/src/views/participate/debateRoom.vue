  ...
  >
    ...
    <div class="call-to-moderator">
      ...
      >
        ...
        <rest-time v-if="rest" @emit-rest="EmitRest"></rest-time>
      </div>
      <div class="call-to-moderator-inner" :style="customCaroselStyle"></div>
    </div>
    <div class="call-to-moderator-blank"></div>
...
export default {
  ...
  methods: {
      ...
      await this.$store.dispatch("makeSessionRoom", index).then(response => {
        ...
        for (var key in data) {
          if (data[key].token.includes("agree")) {
            this.sendSessionAgreeFunc(
              { connectionId: data[key].connectionID },
              data[key].token
            );
          } else {
            this.sendSessionDisagreeFunc(
              { connectionId: data[key].connectionID },
              data[key].token
            );
          }
    ...
    async audioMute(status) {
      let agreeArr = [];
      let disagreeArr = [];
      await this.getAgreePosition().then(res => {
        agreeArr = res;
      });
      await this.getDisagreePosition().then(res => {
        disagreeArr = res;
      });
